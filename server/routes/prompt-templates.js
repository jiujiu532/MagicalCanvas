...
## 对话密度与连贯（强制）
- 剧本是**对话驱动**的短剧：至少 **40%** 的节拍必须含角色对白，格式 角色名说："台词"。
- **对话链条**：同一场问答/争论/试探，A 说 → B 反应/回话 → A 再回应，必须在**相邻节拍**连续呈现，禁止跳接或省略中间回合。
- 能用一句推进剧情或人物关系的对白，就不要写成纯动作描述；无对白时才纯动作。
- 每 3~5 拍至少有一轮**双人对话**，让剧情"说"出来；关键转折、冲突、揭秘必须靠对白点明。
...
## 台词承接（强制）
- dialogue 字段必须从【剧本】原文**逐字摘录**对应台词，禁止改写、概括或留空。
- 有对白的镜头不得省略 dialogue；同一镜头合并多句时用换行分隔，保持 A→B 说话顺序。
- 分镜合并节拍时，该镜头 dialogue 要包含该时段剧本里的**全部**相关对白，不要只取第一句。
...
【场景】环境与空间一句话
【画面】按时长分 2-3 拍铺满整个基准时长，每拍：起止秒 + 谁在画面什么位置朝向 + 做什么连续动作（拍数与覆盖秒数要加起来等于该镜头时长，不要只写前 2 秒）
【人物】每个出场角色一行：角色名：【表情】…【动作】…
【台词】可含本镜头内的连续多句（按发生顺序逐行写）：角色名：（语气）"台词"；无则写 无
【音效】环境音/动作音；无则写 无
【氛围】光影/氛围一句话
...
只输出一个 JSON 对象，禁止任何解释或 markdown：
{
  ...
  "shots": [{
    ...
    "duration": 4,
    "dialogue": "该镜头内的台词原文(可含连续多句,多句之间用空格或换行分隔;用于字幕/配音),无则空字符串"
  }]
}`;
}
